...
function mergeHistories(local, incoming) {
  ...
  if (merged.length > MAX_HISTORY) return merged.slice(0, MAX_HISTORY)
  return merged
}
...
export default function BackupSync({ history, onHistoryChange }) {
  ...
  return (
    <>
      ...
      {/* Modal backdrop — fixed fullscreen overlay */}
      {modal && (
        <div
          style={{
            position: 'fixed',
        ...
        >
          {/* Inner wrapper centres the card both axes, min-height ensures it works even when taller than viewport */}
          <div
            style={{
              minHeight: '100%',
              display: 'flex',
          ...
          >
            <div style={{
              ...
              position: 'relative',
              // Prevent the card itself from shrinking on short viewports
              flexShrink: 0,
            }}>
              {/* Close button */}
              <button
